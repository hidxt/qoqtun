...
func TestValidateServerInvalid(t *testing.T) {
	cases := []struct {
		...
		{"max_tunnels too high", func(c *config.ServerConfig) { c.Policy.MaxTunnelsPerClient = 1025 }},
		{"max_conns_per_client negative", func(c *config.ServerConfig) { c.Policy.MaxConnsPerClient = -1 }},
		...
		{"allowed_targets invalid entry", func(c *config.ServerConfig) { c.Policy.AllowedTargets = []string{"not-a-target"} }},
		{"udp_max_sessions negative", func(c *config.ServerConfig) { c.Policy.UDPMaxSessionsPerTunnel = -3 }},
		...
		{"heartbeat timeout negative", func(c *config.ServerConfig) { c.Heartbeat.TimeoutS = -1 }},
		{"heartbeat miss negative", func(c *config.ServerConfig) { c.Heartbeat.MissThreshold = -1 }},
		{"ca_validity too high", func(c *config.ServerConfig) { c.PKI.CAValidityYears = 101 }},
		{"cert validity negative", func(c *config.ServerConfig) { c.PKI.ClientCertValidityDays = -1 }},
